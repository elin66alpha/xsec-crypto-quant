...
| **合约元信息 / 永续日历** | 动态池候选集 | OKX instruments + 手工核验退市 fallback | `metadata/okx_perp_calendar.json` / `load_perp_calendar()` | 当前 OKX + 已核验退市样本 | 免费 | ⚠️ 见说明② | 进回测；优先用日历上市/退市日，不再只依赖现存列表 |
...
应对：
- 已提交小型永续日历 `metadata/okx_perp_calendar.json`，包含当前 OKX USDT 永续和核验过的退市 fallback 样本（FTT/SRM/ANC）。`data/universe.py` 优先使用日历上市/退市日，退市日只用于截断退市后的月份，不作为退市前的排序信号。
- 探测结果显示，OKX 对 FTT/SRM/ANC 等已下架合约的历史 K 线和 funding REST 请求返回业务错误（无可用历史）；因此这些样本的 OHLCV 走 Binance Vision USDT-M daily klines 归档 fallback。
...
- 早期年份仍不能声称“完整无偏”：当前日历是代表性退市样本，不是完整退市合约全集。`data.validate.check_required_historical_members` 用于阻止 2021/2022 样本完全没有现已退市代表标的的情况。
...
- 扩展退市日历到完整历史合约全集；当前 FTT/SRM/ANC 只是代表性 fallback 样本。
